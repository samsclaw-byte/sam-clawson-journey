    ...
    def do_GET(self):
        ...
        if parsed_url.path == '/oauth/callback':
            ...
            if 'error' in query_params:
                ...
                self.wfile.write(f'<html><body><h2>Error: {error}</h2><p>{error_desc}</p></body></html>'.encode('utf-8'))
            ...
            if returned_state != expected_state:
                ...
                self.send_header('Content-type', 'text/html')
                self.end_headers()
                self.wfile.write('<html><body><h2>Error: Invalid state parameter</h2></body></html>'.encode('utf-8'))
                self.server.auth_code = None
                return
            ...
            if 'code' in query_params:
                ...
                self.send_header('content-type', 'text/html')
                self.end_headers()
                self.wfile.write('<html><body><h2>Error: No authorization code received</h2></body></html>'.encode('utf-8'))
...
def start_oauth_flow():
    ...
    # OAuth URLs (CORRECT - no /developer in OAuth paths)
